...
do
		...
		if [ "`ip r list | grep default | awk '{print $5}'`" = "eth0" ]; then
			sudo ip r delete default via 192.168.159.10 dev eth0
			sudo ip r add default via 192.168.43.1 dev wlan0
                fi
			...
			sudo ip r delete default via 192.168.43.1 dev wlan0
		...
		do
			if [ "`ip r list | grep default | awk '{print $5}'`" = "" ]; then	# Revisa que eth0 tenga ruta si está presente
				sudo ip r add default via 192.168.159.10 dev eth0			# Ojo que no pise al 3G cuando se active, revisar
			fi
